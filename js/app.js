...
import { newAttempt,setResponse,toggleFlag,submitAttempt,saveAttempt,loadAttempt,clearAttempt } from "./core/session.js";
import { scoreAttempt } from "./core/scoring.js";
import { renderControl } from "./renderers.js";
import { createCalculator } from "./calculator.js";

const root=document.querySelector("#app"); let state={view:"home",assessment:null,session:null,items:[],attempt:null};
...
function start(sessionId){ const a=state.assessment, items=drawPracticeSession(bankFor(a),sessionId,{maxItems:12}); const attempt=newAttempt(a.id,sessionId,items); saveAttempt(attempt); setState({view:"question",session:sessionId,items,attempt}); }
function resumeAttempt(){ const a=loadAttempt(); if(!a||a.submitted)return; const assessment=getAssessment(a.assessmentId), bank=getBank(a.assessmentId), byId=new Map(bank.map(i=>[i.id,i])), items=a.itemIds.map(id=>byId.get(id)).filter(Boolean); if(!assessment||items.length!==a.itemIds.length){clearAttempt();return home();} setState({view:"question",assessment,session:a.sessionId,items,attempt:a}); }
function questionView(){
  const a=state.attempt, item=state.items[a.index], session=currentSession();
  root.innerHTML=`<header class="test-head"><div><strong>${state.assessment.label}</strong><span>Session ${state.session} • Untimed${session?.calculatorAllowed?` • Calculator allowed`:""}</span></div><div class="test-tools">${session?.calculatorAllowed?`<button id="calculator-toggle" class="secondary">Calculator</button>`:""}<button id="home" class="link-button">Exit</button></div></header><div id="calculator-slot"></div><div class="test-layout"><aside class="navigator" aria-label="Question navigator">${state.items.map((q,i)=>`<button data-jump="${i}" class="${i===a.index?"current":""} ${a.responses[q.id]!==undefined?"answered":""}">${i+1}${a.flags[q.id]?" ⚑":""}</button>`).join("")}</aside><main class="question-panel">${item.stimulus?`<article class="stimulus"><h2>${item.stimulus.title}</h2>${item.stimulus.text.split("\n").map(p=>`<p>${p}</p>`).join("")}</article>`:""}<p class="question-number">Question ${a.index+1} of ${state.items.length} • ${item.points} point${item.points===1?"":"s"}</p><h1 class="prompt">${item.prompt}</h1><div id="control"></div><div class="question-actions"><button id="flag" class="secondary">${a.flags[item.id]?"Unflag":"Flag for review"}</button><div><button id="prev" class="secondary" ${a.index===0?"disabled":""}>Previous</button>${a.index===state.items.length-1?`<button id="submit">Review & submit</button>`:`<button id="next">Next</button>`}</div></div></main></div>`;
  ...
  const calc=root.querySelector("#calculator-toggle"); if(calc) calc.onclick=()=>{const slot=root.querySelector("#calculator-slot"); if(slot.childElementCount){slot.replaceChildren();calc.setAttribute("aria-expanded","false");}else{slot.append(createCalculator(session.calculatorLevel||"four-function"));calc.setAttribute("aria-expanded","true");}};
