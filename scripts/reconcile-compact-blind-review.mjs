...
if(!input)throw new Error("Usage: node scripts/reconcile-compact-blind-review.mjs path/to/compact-decisions.json");
...
if(sealed.manualRubrics.length){
  throw new Error(`${compact.assessmentId}: compact auto-scored reconciliation cannot certify manual rubrics; use the explicit manual-rubric phase`);
}
const reconciliation=reconcileCleanRoomReview(sealed);
console.log(`COMPACT_REVIEW_RECEIPT:${JSON.stringify({assessmentId:compact.assessmentId,browserEffectiveFingerprint:template.browserEffectiveFingerprint,itemCount:template.itemCount,completedBlindReviewSha256,completedBlindReviewBytes,blindReviewFingerprint:sealed.blindReviewFingerprint,reconciliation})}`);
